...
const Table = class {
  constructor(hostRef) {
    ...
    this.goatCellClick = index.createEvent(this, "goat:table-cell-click", 7);
    this.goatSelectChange = index.createEvent(this, "goat:table-select-change", 7);
    this.goatSort = index.createEvent(this, "goat:sort", 7);
    this.goatPage = index.createEvent(this, "goat:page", 7);
    /**
  ...
  renderHeader() {
    ...
    if (this.selectionType === 'checkbox') {
      fixedCols.push(index.h("div", { class: 'col center' }, index.h("div", { class: 'col-content' }, index.h("goat-checkbox", { class: 'checkbox light', size: 'sm', value: this.isSelectAll, "onGoat:change": this.onSelectAllClick }))));
    }
    this.columns.forEach((col) => {
      let colWidth = DEFAULT_CELL_WIDTH;
      if (col.width)
        colWidth = parseInt(col.width);
      const colEl = index.h("div", { class: { 'col': true, 'sort': this.sortBy === col.name }, style: { width: colWidth + 'rem' } }, index.h("div", { class: 'col-content' }, index.h("div", { class: 'col-text' }, col.label), index.h("div", { class: 'col-actions' }, (() => {
        if (!this.sortable)
          return;
        ...
        return index.h("goat-button", { size: 'sm', icon: icon, class: 'col-action color-secondary', variant: 'ghost', onClick: () => {
            if (this.sortBy === col.name) {
              if (this.sortOrder === 'asc')
                this.sortOrder = 'desc';
              else
    ...
    return index.h("div", { class: 'header' }, index.h("div", { class: 'row' }, index.h("div", { class: 'fixed-columns columns-container' }, fixedCols), index.h("div", { class: 'scrollable-columns columns-container' }, scrollCols)));
  }
  renderBody() {
    const rows = [];
    let data = [...this.data];
    ...
    data.forEach((row) => {
      ...
      const scrollCols = [];
      if (this.selectionType === 'checkbox')
        fixedCols.push(index.h("div", { class: { 'col': true, 'center': true } }, index.h("div", { class: 'col-content' }, index.h("goat-checkbox", { class: 'checkbox light', size: 'sm', value: this.selectedRowKeys.includes(row[this.keyField]), "onGoat:change": () => this.onRowSelectClick(row) }))));
      this.columns.forEach((column) => {
        let colWidth = DEFAULT_CELL_WIDTH;
        if (column.width)
          colWidth = parseInt(column.width);
        const colEl = index.h("div", { tabindex: '1', class: { 'col': true, 'col-hover': (this.hoveredCell.row === row && this.hoveredCell.column === column) }, style: { width: colWidth + 'rem' }, onMouseOver: () => this.onCellMouseOver(row, column), onClick: () => {
            const selection = window.getSelection();
            if (selection.type != 'Range')
              this.onCellClick(row, column);
          } }, index.h("div", { class: 'col-content' }, index.h("div", { class: 'col-text', title: row === null || row === void 0 ? void 0 : row[column.name] }, row === null || row === void 0 ? void 0 : row[column.name])));
  ...
  renderPagination() {
    if (this.paginate)
      return index.h("div", { class: 'pagination' }, index.h("div", { class: 'page-sizes-select' }, index.h("goat-form-control", { label: 'Items per page:', inline: true, class: 'form-control' }, index.h("goat-select", { size: 'sm', class: 'select', items: SUPPORTED_PAGE_SIZES, positions: 'top-right', value: this.pageSize, "onGoat:change": (e) => {
          this.pageSize = e.detail.value;
          this.goatPage.emit({ page: this.page, pageSize: this.pageSize });
        } }))), index.h("div", { class: 'pagination-item-count' }, index.h("goat-text", { size: 'sm' }, this.pageSize * (this.page - 1), " - ", this.pageSize * (this.page) < this.getTotalItems() ? this.pageSize * (this.page) : this.getTotalItems(), " of ", this.getTotalItems(), " items")), index.h("div", { class: 'pagination-right' }, index.h("div", { class: 'table-footer-right-content' }, index.h("div", { class: 'table-footer-right-content-pagination' }, index.h("goat-button", { size: 'sm', icon: 'arrow-left', variant: 'ghost', disabled: this.page === 1, onClick: () => {
          this.page = this.page - 1;
          this.goatPage.emit({ page: this.page, pageSize: this.pageSize });
        } }), index.h("goat-button", { size: 'sm', icon: 'arrow-right', variant: 'ghost', disabled: this.pageSize * (this.page) >= this.getTotalItems(), onClick: () => {
          this.page = this.page + 1;
          this.goatPage.emit({ page: this.page, pageSize: this.pageSize });
        } })))));
  }
  render() {
    return index.h(index.Host, null, index.h("div", { class: { 'table': true, 'sortable': this.sortable, 'paginate': this.paginate } }, index.h("div", { class: 'table-scroll-container' }, this.renderHeader(), (this.data.length) ? this.renderBody() : this.renderEmptyState()), index.h("div", { class: 'table-footer' }, this.renderPagination())));
  }
  renderEmptyState() {
    return index.h("div", { class: 'empty-table' }, index.h("goat-empty-state", Object.assign({ class: 'content-center' }, this.emptyState)));
  }
  get elm() { return index.getElement(this); }
};
Table.style = tableCss;

exports.goat_table = Table;

//# sourceMappingURL=goat-table.cjs.entry.js.map