...
const Dropdown = class {
  ...
  listenMenuItemClick(evt) {
    if (utils.isEventTriggerByElement(evt, this.elm)) {
      this.closeList();
    }
    this.isOpen = false;
  }
  ...
  renderItems() {
    if (this.items)
      return index.h("goat-menu", { class: 'items' }, this.items.map((item) => {
        return index.h("goat-menu-item", { value: item.value, tabindex: this.isOpen ? '0' : '-1' }, item.icon && index.h("goat-icon", { name: item.icon, slot: 'start', size: 'sm' }), item.label, item.hint && index.h("span", { slot: 'end' }, item.hint));
      }));
  }
  render() {
    return (index.h(index.Host, { "has-focus": this.hasFocus, "is-open": this.isOpen }, index.h("div", { class: {
        'dropdown': true,
        [this.position]: true,
        'is-open': this.isOpen,
        ...
        this.toggleList();
      } }, index.h("div", { class: 'slot-container' }, index.h("slot", null))), index.h("div", { class: 'dropdown-content' }, this.renderItems(), index.h("slot", { name: 'dropdown-content' })))));
  }
  get elm() { return index.getElement(this); }
};
Dropdown.style = dropdownCss;

exports.goat_dropdown = Dropdown;

//# sourceMappingURL=goat-dropdown.cjs.entry.js.map